    ...
    <div style={{ padding: '5px 0', borderTop: HAIRLINE }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline' }}>
        <span style={{ fontSize: 12, fontWeight: 700, color: INK.text, fontFamily: MONO }}>
          {p.symbol}
          {p.adopted && (
            <span style={{ fontSize: 8, color: AMBER, marginLeft: 6, letterSpacing: '0.1em' }}>
              ADOPTED
            </span>
          )}
          {p.state === 'exiting' && (
      ...
      >
        ...
        <span style={{ color: INK.dim }}>{p.broker_stop ? 'broker stop' : 'software stop'}</span>
    ...
    <div style={{ padding: '9px 14px 8px', borderTop: HAIRLINE }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 7 }}>
        ...
        <span
          style={{
            ...
            fontWeight: 700,
            fontFamily: MONO,
            color: pnlColor(mode.realized_today),
          }}
        >
      ...
      {mode.closed_today.map((c, i) => (
        ...
        >
          ...
          <span style={{ fontSize: 10.5, fontFamily: MONO, color: pnlColor(c.pnl) }}>
            {fmtMoney(c.pnl)}
          </span>
        </div>
      ))}
    ...
    <Panel id="panel-trading" style={{ height: '100%' }}>
            ...
            <span style={{ display: 'inline-flex', alignItems: 'center', gap: 5 }}>
              <span
                style={{
                  ...
                  animation: trading.market.is_open
                    ? 'led-pulse 1.6s ease-in-out infinite'
                    : undefined,
                }}
              />
              <span
                style={{
                  ...
                  color: trading.market.is_open ? GOLD : INK.dim,
                  fontFamily: MONO,
                }}
              >
      ...
      {!trading && (
        <EmptyState>
          ...
          <span style={{ color: UI.soft }}> make demo</span>) and both books render here with
          positions, stops, and reasoning.
        </EmptyState>
      )}
        ...
        <div style={{ overflowY: 'auto', minHeight: 0 }}>
              ...
              <>
                <div style={{ fontSize: 11, fontFamily: MONO }}>
                  ...
                  {trading.last_decision.symbol && (
                    <span style={{ color: INK.text, marginLeft: 6 }}>
                      {trading.last_decision.symbol}
                    </span>
                  )}
                  <span style={{ color: INK.dim, marginLeft: 6, fontSize: 9 }}>
                    {trading.last_decision.at
                      ? new Date(trading.last_decision.at).toLocaleTimeString([], { hour12: false })
                      : ''}
                  </span>
                </div>
          ...
          <div style={{ padding: '9px 14px 12px', borderTop: HAIRLINE }}>
            ...
            {trading.alerts.length === 0 && (
              <div style={{ fontSize: 10, color: INK.dim, fontFamily: MONO }}>
